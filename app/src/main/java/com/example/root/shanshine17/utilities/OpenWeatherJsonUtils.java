...
public class OpenWeatherJsonUtils {
     ...
     * function, leveraging the data we have stored in the JSON. for now, we just convert the JSON into
    ...
    public static String[] getSimpleWeatherStringFromJson(Context context, String forecastJsonStr)
        ...
        for (int i = 0; i < weatherArray.length(); i++) {
            ...
            /**
             * We ignore alll the datetime values embedded in the JSON and assume that
             * the values are returned in order by day (Which is not guaranteed to be correct)
             */
            dateTimeMillis = startDay + SunshineDateUtils.DAY_IN_MILLIS * i;
            date = SunshineDateUtils.getFriendDateString(context, dateTimeMillis, false);

            /**
             * Description is in a child array called "weather" , which is 1 element long
             * that element also contains a weaher code
             */
